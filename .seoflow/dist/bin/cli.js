...
// .seoflow/lib/openrouter-client.ts
function getApiKey() {
...
// .seoflow/lib/ubersuggest-client.ts
import fs9 from "fs";
...
// .seoflow/lib/python/python-manager.ts
...
// .seoflow/lib/content-quality/content-quality.ts
...
// .seoflow/pipeline/content-quality.ts
...
// .seoflow/lib/reports/pdf-generator.ts
...
// .seoflow/lib/reports/reports.ts
...
// .seoflow/pipeline/report-export.ts
...
async function interactiveInit() {
  ...
  const configPath = path13.join(cwd, "seoflow.config.json");
  ...
  const dataDir = path13.join(cwd, ".seoflow", "data");
  if (!fs15.existsSync(dataDir)) {
  ...
  try {
    ...
    if (fs15.existsSync(pkgPath)) {
      ...
      fs15.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + "\n");
...
async function main() {
  ...
  switch (command) {
    case "init":
      ...
      break;
    case "run":
      const { runPipeline: runPipeline2 } = await Promise.resolve().then(() => (init_run(), run_exports));
