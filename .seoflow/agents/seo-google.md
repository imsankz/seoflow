...
1. Check credentials: `python3 .seoflow/scripts/google_auth.py --check --json`
2. Determine tier (0 = API key, 1 = + service account, 2 = + GA4)
3. Execute tier-appropriate analysis
4. Format output to match seoflow conventions

## Tier-Based Workflow

### Tier 0 (API Key Only)
- Run PSI + CrUX on homepage: `python3 .seoflow/scripts/pagespeed_check.py <url> --json`
- Run CrUX History for origin: `python3 .seoflow/scripts/crux_history.py <origin> --origin --json`
- Report CWV field data with traffic-light ratings

### Tier 1 (+ Service Account)
- All Tier 0 checks
- GSC top queries/pages (28 days): `python3 .seoflow/scripts/gsc_query.py --property <prop> --json`
- URL Inspection on homepage + key pages: `python3 .seoflow/scripts/gsc_inspect.py <url> --json`
- GSC sitemap status: `python3 .seoflow/scripts/gsc_query.py sitemaps --property <prop> --json`

### Tier 2 (Full)
- All Tier 1 checks
- GA4 organic traffic (28 days): `python3 .seoflow/scripts/ga4_report.py --property <id> --json`
- Top organic landing pages: `python3 .seoflow/scripts/ga4_report.py --property <id> --report top-pages --json`
...
```bash
python3 .seoflow/scripts/google_report.py --type full --data data.json --domain DOMAIN --format pdf --json
```
