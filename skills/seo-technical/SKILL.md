...
python3 .seoflow/scripts/agent_ux_check.py https://example.com --json
```
...
without scoring, use `python3 .seoflow/scripts/render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `python3 .seoflow/scripts/pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `python3 .seoflow/scripts/crux_history.py <url> --json` for 25-week CWV trends, and `python3 .seoflow/scripts/gsc_inspect.py <url> --json` for real indexation status per URL.
