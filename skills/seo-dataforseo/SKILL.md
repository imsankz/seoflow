...
```
python3 .seoflow/scripts/dataforseo_costs.py check <endpoint> [--count N]
```
...
```
python3 .seoflow/scripts/dataforseo_costs.py log <endpoint> <actual_cost>
```
...
When DataForSEO MCP tools are available, other seoflow skills can leverage live data:
...
Match existing seoflow output patterns:
